...
#                     menuSubItem("Comparando Preços", tabName = "aba1261"),
             ...
             menuItem("Simulação", icon = icon("angle-right"),tabName = "aba1263")),
...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem("aba1263",
                      fluidPage(
                        box(status = "primary",title = "Simulação", solidHeader =T, width = "50%",
                            titlePanel(""),
                            plotOutput("aba1263.plot"),
                            column(4,
                                   box(status = "primary",title = "Análise de solo", solidHeader =T, width = "50%",
                                       numericInput("res.aba1263.1","Conteúdo de Fósforo (mg/dm3):",min = 0,value =5),
                                       numericInput("res.aba1263.2","Conteúdo de Potássio (mmolc/dm3):",min = 0,value =1.2,step = 0.1),
                                       numericInput("res.aba1263.3","Conteúdo de Cálcio (mmolc/dm3):",min = 0,value =5),
                                       numericInput("res.aba1263.4","Conteúdo de Magnésio (mmolc/dm3):",min = 0,value =5),
                                       uiOutput("res.aba1263.a0")
                                       )),
                            column(4,
                                   box(status = "primary",title = "Adubação e calagem", solidHeader =T, width = "50%",
                                       numericInput("res.aba1263.5","Dose de Fósforo (kg/ha):",min = 0,value =10),
                                       numericInput("res.aba1263.6","Dose de Potássio (kg/ha):",min = 0,value =10),
                                       numericInput("res.aba1263.7","Dose de calcário (t/ha)",min = 0,value =0),
                                       numericInput("res.aba1263.8","Conteúdo de CaO do calcário (%)",min = 0,value =42),
                                       numericInput("res.aba1263.9","Conteúdo de MgO do calcário (%)",min = 0,value =13)
                                        )),
                            column(4,
                                   box(status = "primary",title = "Cenário simulado", solidHeader =T, width = "50%",
                                       uiOutput("res.aba1263.10"),
                                       uiOutput("res.aba1263.11"),
                                       uiOutput("res.aba1263.12"),
                                       uiOutput("res.aba1263.13")))
                        ))),
    # #####################################################################################################################################################################################
                  ...
                  sidebarLayout(
                    sidebarPanel(
                      box(status = "primary",title = "Análise de Solo", solidHeader =T, width = "50%",
                          ...
                          numericInput("FR221.4","Capacidade de Troca Catiônica Efetiva (CTCe) (mmolc/dm3)",min = 0,value =80),
              ...
              box(status = "primary",title = "Fosfatagem", solidHeader =T, width = "50%",
                  ...
                  sidebarLayout(
                    ...
                    mainPanel(
                      ...
                      hr(),
                      img(src='tabela_fosfatagem.jpg',width = "100%",height = "100%")
                  ))))),
...
shinyApp(
  ...
  server = function(input, output) { 
    ...
    data.aba1263<-reactive({
      data1<-read.table(file = "simula.txt",header = T,sep = "\t")
      LISTA<-list(data1)
      return(LISTA)
    })
    
    
    output$res.aba1263.a0<-renderUI({
      data<-data.aba1263()[[1]]
      escolha<-unique(data[as.character(data[,1])=="P",2])
      selectInput("res.aba1263.a1", "Escolha a Cultura:",
                  choices = c(as.character(escolha)))
    })
    
    res.aba1263.a.P<-reactive({ #classe atual
      data<-data.aba1263()[[1]]
      min<-unique(data[as.character(data[,1])=="P"&as.character(data[,2])==input$res.aba1263.a1,3])
      max<-unique(data[as.character(data[,1])=="P"&as.character(data[,2])==input$res.aba1263.a1,4])
      Val.A<-input$res.aba1263.1
      classe.A<-as.character(unique(data[as.character(data[,1])=="P"&as.character(data[,2])==input$res.aba1263.a1,5]))[((Val.A)>=min)&((Val.A)<=max)]
      classe.A2<-as.character(unique(data[as.character(data[,1])=="P"&as.character(data[,2])==input$res.aba1263.a1,6]))[((Val.A)>=min)&((Val.A)<=max)]
      Val.N<-input$res.aba1263.5*0.2185+Val.A
      classe.N<-as.character(unique(data[as.character(data[,1])=="P"&as.character(data[,2])==input$res.aba1263.a1,5]))[((Val.N)>=min)&((Val.N)<=max)]
      classe.N2<-as.character(unique(data[as.character(data[,1])=="P"&as.character(data[,2])==input$res.aba1263.a1,6]))[((Val.N)>=min)&((Val.N)<=max)]
      fim<-rbind(c("P","Antes",Val.A,classe.A,classe.A2),c("P","Depois",Val.N,classe.N,classe.N2))
      return(fim)
    })
    
    res.aba1263.a.K<-reactive({ #classe atual
      data<-data.aba1263()[[1]]
      min<-unique(data[as.character(data[,1])=="K",3])
      max<-unique(data[as.character(data[,1])=="K",4])
      Val.A<-input$res.aba1263.2
      classe.A<-as.character(unique(data[as.character(data[,1])=="K",5]))[((Val.A)>=min)&((Val.A)<=max)]
      classe.A2<-as.character(unique(data[as.character(data[,1])=="K",6]))[((Val.A)>=min)&((Val.A)<=max)]
      Val.N<-input$res.aba1263.6*0.415+Val.A
      classe.N<-as.character(unique(data[as.character(data[,1])=="K",5]))[((Val.N)>=min)&((Val.N)<=max)]
      classe.N2<-as.character(unique(data[as.character(data[,1])=="K",6]))[((Val.N)>=min)&((Val.N)<=max)]
      fim<-rbind(c("K","Antes",Val.A,classe.A,classe.A2),c("K","Depois",Val.N,classe.N,classe.N2))
      return(fim)
    })
    
    res.aba1263.a.Ca<-reactive({ #classe atual
      data<-data.aba1263()[[1]]
      min<-unique(data[as.character(data[,1])=="Ca",3])
      max<-unique(data[as.character(data[,1])=="Ca",4])
      Val.A<-input$res.aba1263.3
      classe.A<-as.character(unique(data[as.character(data[,1])=="Ca",5]))[((Val.A)>=min)&((Val.A)<=max)]
      classe.A2<-as.character(unique(data[as.character(data[,1])=="Ca",6]))[((Val.A)>=min)&((Val.A)<=max)]
      Val.N<-input$res.aba1263.7*input$res.aba1263.8*0.1785+Val.A
      classe.N<-as.character(unique(data[as.character(data[,1])=="Ca",5]))[((Val.N)>=min)&((Val.N)<=max)]
      classe.N2<-as.character(unique(data[as.character(data[,1])=="Ca",6]))[((Val.N)>=min)&((Val.N)<=max)]
      fim<-rbind(c("Ca","Antes",Val.A,classe.A,classe.A2),c("Ca","Depois",Val.N,classe.N,classe.N2))
      return(fim)
    })
    
    res.aba1263.a.Mg<-reactive({ #classe atual
      data<-data.aba1263()[[1]]
      min<-unique(data[as.character(data[,1])=="Mg",3])
      max<-unique(data[as.character(data[,1])=="Mg",4])
      Val.A<-input$res.aba1263.4
      classe.A<-as.character(unique(data[as.character(data[,1])=="Mg",5]))[((Val.A)>=min)&((Val.A)<=max)]
      classe.A2<-as.character(unique(data[as.character(data[,1])=="Mg",6]))[((Val.A)>=min)&((Val.A)<=max)]
      Val.N<-input$res.aba1263.7*input$res.aba1263.9*0.2482+Val.A
      classe.N<-as.character(unique(data[as.character(data[,1])=="Mg",5]))[((Val.N)>=min)&((Val.N)<=max)]
      classe.N2<-as.character(unique(data[as.character(data[,1])=="Mg",6]))[((Val.N)>=min)&((Val.N)<=max)]
      fim<-rbind(c("Mg","Antes",Val.A,classe.A,classe.A2),c("Mg","Depois",Val.N,classe.N,classe.N2))
      return(fim)
    })
    
    
    output$res.aba1263.10<-renderUI({
      selectInput("a.2112","Conteúdo de Fósforo (mg/dm3):", choices = res.aba1263.a.P()[2,3])
    })
    
    output$res.aba1263.11<-renderUI({
      selectInput("a.2112","Conteúdo de Potássio (mmolc/dm3):", choices = res.aba1263.a.K()[2,3])
    })
    
    output$res.aba1263.12<-renderUI({
      selectInput("a.2112","Conteúdo de Cálcio (mmolc/dm3):", choices = res.aba1263.a.Ca()[2,3])
    })
    
    output$res.aba1263.13<-renderUI({
      selectInput("a.2112","Conteúdo de Magnésio (mmolc/dm3):", choices = res.aba1263.a.Mg()[2,3])
    })
    
    
    output$aba1263.plot<-renderPlot({
      
      dados22<-rbind(res.aba1263.a.P(),
                     res.aba1263.a.K(),
                     res.aba1263.a.Ca(),
                     res.aba1263.a.Mg())
      dados<-as.data.frame(dados22)
      
      colnames(dados)<-c("elem","sim","val","classe","classe2")
      
      dados$val<-as.numeric(as.character(dados$val))
      dados$classe2<-as.numeric(as.character(dados$classe2))
      dados$elem<-as.factor(dados$elem)
      dados$sim<-as.factor(dados$sim)
      dados$elem1<-factor(dados$elem,levels = c("P","K","Ca","Mg"))
      dados$classe<-as.character(dados$classe)
        
      dados$nivel<-as.factor(dados$classe)
      nivel<-as.character(dados$nivel)
      
      ggplot(dados, aes(x = elem1 , y = classe2,fill=classe))+
        geom_bar(stat = "identity",
                 position = 'dodge') +
        scale_fill_grey(start = 0.2, end = 0.9)+
        facet_grid(. ~ sim) +
        geom_text(aes(label=as.character(nivel),mapping=F), position=position_dodge(width=0.9), vjust=-0.20,size=5)+
        #geom_text(aes(label=classe), position=position_dodge(width=0.9), vjust=-0.20,size=5)+
        theme_bw()+
        xlab("")+
        ylab("Classe de Teor")+
    ...
    data.aba2213<-reactive({
      data1<-read.table(file = "fosfatagem1.txt",header = T,sep = "\t")
      LISTA<-list(data1)
      return(LISTA)
    })
    ...
    output$aba1273.per.2<-renderUI({
      if(input$aba1273.11=="Personalizado"){
        numericInput("aba1273.out.1","Conteúdo de Nitrogênio (N, g/kg):",min = 0,value = 10,step = 1)
      }})
    
    output$aba1273.per.3<-renderUI({
      if(input$aba1273.11=="Personalizado"){
        numericInput("aba1273.out.2","Conteúdo de Fósforo (P, g/kg):",min = 0,value = 20,step = 1)
      }})
    
    output$aba1273.per.4<-renderUI({
      if(input$aba1273.11=="Personalizado"){
        numericInput("aba1273.out.3","Conteúdo de Potássio (K, g/kg):",min = 0,value = 30,step = 1)
      }})
    
    output$aba1273.per.5<-renderUI({
      if(input$aba1273.11=="Personalizado"){
        numericInput("aba1273.out.4","Conteúdo de Cálcio (Ca, g/kg):",min = 0,value = 5,step = 1)
      }})
    
    output$aba1273.per.6<-renderUI({
      if(input$aba1273.11=="Personalizado"){
        numericInput("aba1273.out.5","Conteúdo de Magnésio (Mg, g/kg):",min = 0,value = 2,step = 1)
      }})
    
    output$aba1273.per.7<-renderUI({
      if(input$aba1273.11=="Personalizado"){
        numericInput("aba1273.out.6","Conteúdo de Enxofre (S, g/kg):",min = 0,value = 20,step = 1)
      }})
    ...
    output$res.aba211.8<-renderUI({
      ...
      selectInput("res.aba2111.8", "Conteúdo de Fósforo (mg/dm3):",
                  choices = c(as.character(escolha)))
    })
    ...
    output$res.aba214.3<-renderUI({
      ...
      textInput("a.2141","Classe de acidez (pH em CaCl2):", value = escolha)
    })
    ...
    output$res.aba23.3<-renderUI({
      ...
      if(!input$res.aba23.1%in%c("Eucalyptus","Pinus")){
        ...
        if(input$res.aba23.1%in%c("Soja")){
          return(NULL)}
        selectInput("res.aba23.31", "Classe de Resposta ao Nitrogênio (N):",
                    choices = c(as.character(escolha)))}
    })
